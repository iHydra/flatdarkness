...
var themeColor, quotedColor, showLogo, enableSFW, hideMenu, showTime, enableBadge;
chrome.storage.sync.get({
  ...
  if (!chrome.runtime.error) {
    ...
    quotedColor = settings.quoteColor;
    prefixColor = settings.prefixColor;
    showLogo = settings.showLogo;
    enableSFW = settings.enableSFW;
...
if (window.location.href != "https://hackforums.net/misc.php?action=buddypopup") {
  var profileLink = $('#panel > strong:nth-child(1) > a:nth-child(1)').attr("href");
  var UserID = profileLink.split("=")[2];
}
var addbuddyIMG = chrome.runtime.getURL('assets/addbuddy.png');
var removebuddyIMG = chrome.runtime.getURL('assets/removebuddy.png');
var addignoreIMG = chrome.runtime.getURL('assets/addignore.png');
var removeignoreIMG = chrome.runtime.getURL('assets/removeignore.png');
$('img[src$="/dismiss_notice.gif"]').attr('src', 'https://i.imgur.com/uxvQQDI.png'); // PM Notif Dismiss Icon
$('img[src$="/add_buddy.gif"]').attr('src', addbuddyIMG); // Add Buddy Icon https://i.imgur.com/3d5FKNX.png
$('img[src$="/remove_buddy.gif"]').attr('src', removebuddyIMG); // Remove Buddy Icon
$('img[src$="/add_ignore.gif"]').attr('src', addignoreIMG); // Add Ignore Icon
$('img[src$="/remove_ignore.gif"]').attr('src', removeignoreIMG); // Remove Ignore Icon
...
var quotedPosts = [];
chrome.storage.local.get("quotedPosts", function(multiQuotes) {
  if (!chrome.runtime.error) {
    quotedPosts = multiQuotes.quotedPosts;
  } else {
    console.log("RUNTIME ERROR 1: REPORT TO iHydra");
  }
});
  ...
  $(".trow1 .button").each(function() {
    ...
    chrome.storage.local.get("quotedPosts", function(multiQuotes) {
      if (!chrome.runtime.error) {
        ...
        console.log("RUNTIME ERROR 2: REPORT TO iHydra");
      }
    });
  });
...
if (window.location.href == "https://hackforums.net/misc.php?action=buddypopup") { // Buddy List Online Status Fix
  ...
  $('img[src$="/buddy_away.gif"]').attr('style', 'position:relative;margin-bottom:7px'); // Away Status
  $('img[src$="/buddy_online.gif"]').attr('style', 'position:relative;margin-bottom:7px'); // Online Status
  $('img[src$="/buddy_offline.gif"]').attr('style', 'position:relative;margin-bottom:7px'); // Offline Status
  $('div[style$="height: 300px;"]').attr('style', 'overflow:auto;'); // Expand Height
}
...
function getSettings() {
  ...
  if (window.location.pathname != "/private.php") {
    ...
    $('blockquote > cite:contains(' + userName + ')').css({
      'color': '#' + quotedColor,
      'font-weight': 'bold',
      'border-bottom': '1px dotted' + '#' + quotedColor
    });
  }
  ...
  if (themeColor != '393939') {
    document.body.style.setProperty('--themeColor', '#' + themeColor);
  }
  if (prefixColor != '046DEF') {
    document.body.style.setProperty('--prefixColor', '#' + prefixColor);
  }
}
if (window.location.pathname == "/private.php") {
...
if (window.location.pathname == "/editpost.php" || window.location.pathname == "/newreply.php" || window.location.pathname == "/newthread.php") {
